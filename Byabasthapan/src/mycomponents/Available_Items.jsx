...
export default function Available_Items() {
    ...
    const fetchavailableData = () => {
        ...
        .then((result) => {
            ...
            if (data.StatusCode === 200) {
                setavailableData(data.listNewdata);
    ...
    const filteredSales = Array.isArray(availableData) ? availableData.filter(sale => {
        return (
    ...
    const sortedSales = mergeSort(filteredSales, sortType === "name" ? "Product_Name" : "Marked_Price", sortType === "name");

    return (
        <div className="container mx-auto p-6">
            <header className="text-center mb-6">
                <h1 className="text-3xl font-bold text-gray-800">Available Items</h1>
            </header>
            ...
            <table className="min-w-full table-auto border-collapse text-center">
                <thead className="bg-gray-100">
                    <tr>
                        ...
                        <th className="py-2 px-4 border border-gray-300">Marked Price</th>
                        <th className="py-2 px-4 border border-gray-300">Manufacture Date</th>
                        <th className="py-2 px-4 border border-gray-300">Expire Date</th>
                    </tr>
                </thead>
                <tbody className="bg-white">
                    {sortedSales.map((sale, i) => (
                        <tr key={i} className="hover:bg-gray-50">
                            ...
                            <td className="py-2 px-4 border border-gray-300">{sale.Marked_Price}</td>
                            <td className="py-2 px-4 border border-gray-300">{sale.ManufactureDate}</td>
                            <td className="py-2 px-4 border border-gray-300">{sale.ExpireDate}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
